...
func (t *topicRepository) GetRotationTopic() []modules.Topic {
	var topics []modules.Topic = nil
	if err := database.DB.Model(&modules.Topic{}).Select("id,title_,image_url").Where("deleted_ = ?", false).Where("published_ = ?",true).Where("rotation_ =?",true).Find(&topics).Error ;err !=nil{
		return nil
	}
	return topics
}
